...
from comparisons.slicegpt.slicegpt import data_utils, gpu_utils, hf_utils, layernorm_fusion, rotate, utils
...
def slicing_arg_parser(interactive: bool = True) -> argparse.Namespace:
    ...
    parser.add_argument("--dtype", type=str, help="Data type to use.", choices=["fp32", "fp16", "bfp16"], default="bfp16")
    parser.add_argument(
        "--cal-dataset",
        type=str,
    ...
    parser.add_argument("--eval_ppl", action="store_true", help="Evaluate the model perplexity after slicing.")
    parser.add_argument("--eval_zeroshot", action="store_true", help="Evaluate the model perplexity after slicing.")
    parser.add_argument("--data_log_path", type=str, help="Path to the data log file.")
...
def slicing_main(args: argparse.Namespace) -> None:
    logger.info("Running SliceGPT experiment.")
    ...
    try:
        ...
        logger.info(f'Failed to initialize wandb: {e}, continuing without wandb')
        wandb.init(project=args.wandb_project, mode='disabled')

    if args.sliced_model_path:
        print(f"Loading sliced model from {args.sliced_model_path}")
    ...
    train_ids = get_trainloaders(args.cal_dataset, 
                                  seed=args.seed,
                                  tokenizer=tokenizer,
                                  **data_configs
    )
    ...
    for input_ids in train_ids:
        ...
        train_loader.append({'input_ids': input_ids, "attention_mask": attention_mask})
    ...
    logger.info(f"Number of tokens in the training data: {total_tokens}")
    ...
    #     logger.info(f'Loaded model perplexity: {dataset_ppl}')
    ...
    #     dataset_ppl = gpu_utils.evaluate_ppl(model, model.config.pad_token_id, test_loader)
    #     logger.info(f'Post-fusion: {dataset_ppl:.4f}')
    ...
    #                 shutil.copy(str(file), sliced_model_dir)
    #         except OSError as e:
    #             logger.info(f'Failed to copy configs and tokenizer files: {e}')

    #     logger.info(f"Saved sliced model to {args.save_dir}")

    # reset_model_device()
    # dataset_ppl = gpu_utils.evaluate_ppl(model, model.config.pad_token_id, test_loader)
    # logger.info(f'After rotating and slicing {dataset_ppl:.4f}')
    ...
    if args.eval_zeroshot:
        ...
        if '30b' or '66b' or '70b' in args.model:
            parallelize = True
        else:
            parallelize = False

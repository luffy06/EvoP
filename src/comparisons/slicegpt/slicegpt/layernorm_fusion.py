...
from comparisons.slicegpt.slicegpt.model_adapter import ModelAdapter
from comparisons.slicegpt.slicegpt.modules import RMSN
    ...
    for linear in linear_layers:
        ...
        if hasattr(layernorm, 'bias'):
            if linear.bias is None:
                linear.bias = Parameter(torch.zeros(linear.out_features, dtype=torch.float64, device=linear.weight.device))
